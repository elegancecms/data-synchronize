$(() => {
        ...
        httpClient
            ...
            .then(async ({ headers, data }) => {
                if (headers['content-disposition'] === undefined) {
                    ...
                    EleganceCMS.showError(responseJson?.message || $form.data('error-message'))
                ...
                EleganceCMS.showSuccess($form.data('success-message'))
            })
            .catch((error) => {
                EleganceCMS.showError(error.message ? (error.statusText + ': ' + error.message) : form.data('error-message'))
            })
    }
    ...
    if ($form.length > 0) {
        ...
        const cleanup = () => {
            EleganceCMS.hideButtonLoading($button)
        ...
        dropzone.on('sending', () => {
            ...
            EleganceCMS.showButtonLoading($button)
        })
